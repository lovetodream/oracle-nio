...
function genWithContextParameter() {
    ...
    echo -n ").Type, context: OracleDecodingContext<JSONDecoder>, file: String = #fileID, line: Int = #line) throws"
    ...
    echo
    echo "        do {"
    echo "            let r0 = try T0._decodeRaw(from: &cellData, type: column.dataType, context: context)"
    echo
    for ((n = 1; n<$how_many; n +=1)); do
        echo "            columnIndex = $n"
        ...
        echo "            let r$n = try T$n._decodeRaw(from: &cellData, type: column.dataType, context: context)"
        echo
    done
    ...
    echo ")"
    echo "        } catch let code as OracleDecodingError.Code {"
    echo "            throw OracleDecodingError("
    echo "                code: code,"
    echo "                columnName: column.name,"
    echo "                columnIndex: columnIndex,"
    echo "                targetType: swiftTargetType,"
    echo "                oracleType: column.dataType,"
    echo "                oracleData: cellData,"
    echo "                file: file,"
    echo "                line: line"
    echo "            )"
    echo "        }"
...
echo "}"
} > "$here/../Sources/OracleNIO/OracleRow-multi-decode.swift"
