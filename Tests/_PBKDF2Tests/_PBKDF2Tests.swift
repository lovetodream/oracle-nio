...
final class _PBKDF2Tests: XCTestCase {
    ...
    // Test inputs from https://www.ietf.org/rfc/rfc6070.txt
    static let cases = [
        TestCase(
        ...
        TestCase(
            p: "password", s: "salt", c: 16_777_216, dkLen: 20,
            expected: (
        ...
        TestCase(
            p: "Password", s: "NaCl", c: 80_000, dkLen: 128,
            expected: (
        ...
        TestCase(
            p: "Password", s: "sa\0lt", c: 4096, dkLen: 256,
            expected: (
    ...
    func testAll() throws {
        ...
        for (index, testCase) in Self.cases.enumerated() where !testCase.skip {
            ...
            let duration = try clock.measure {
                let sha256 = try PBKDF2<SHA256>.calculate(
                    length: testCase.dkLen, password: testCase.p, salt: testCase.s,
                    rounds: testCase.c)
                let sha384 = try PBKDF2<SHA384>.calculate(
                    length: testCase.dkLen, password: testCase.p, salt: testCase.s,
                    rounds: testCase.c)
                let sha512 = try PBKDF2<SHA512>.calculate(
                    length: testCase.dkLen, password: testCase.p, salt: testCase.s,
                    rounds: testCase.c)
