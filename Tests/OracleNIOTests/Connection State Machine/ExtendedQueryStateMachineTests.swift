...
final class ExtendedQueryStateMachineTests: XCTestCase {
    ...
    func testCancellationOnlyOnce() throws {
        ...
        XCTAssertEqual(state.rowDataReceived(.init(slice: .init(data: rowData)), capabilities: .init()), .sendFetch(self.queryContext))
